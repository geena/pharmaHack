...
class OrdersController < ApplicationController
    ...
    if @order.order_name.split(" ").first == "Penicillin"
        @allergy_interaction = '#{@order.order_name.split(" ").first} Allergy for Patient: #{@order.patient_name}!'
    end
    ...
    def cds_hook
      ...
      JSON.parse(response.body)["parameter"].first["part"][1]["valueString"]
    end
